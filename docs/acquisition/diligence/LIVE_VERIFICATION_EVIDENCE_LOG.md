...
### Entry 2026-04-10: Public Host Closeout

- **Date:** 2026-04-10
- **Environment:** Production frontend `https://www.coparrent.com` against production Supabase backend `jnxtskcpwzuxyxjzqrkv`
- **Tester:** admin `coparrenttesting@yahoo.com` (`Parent A`) plus production QA users `access-code-qa-redeem-1775843594826@coparrent.test` and `access-code-qa-inactive-1775843594826@coparrent.test`
- **Scenario:** Deploy the access-code frontend from a clean worktree linked to the real `coparrent` Vercel project, confirm the public production host exposes the `Access Codes` admin tab, mint a fresh two-code QA batch, confirm the raw code reveal is one-time only, verify both previews appear in inventory, redeem one code through the live `/dashboard/settings` route, verify the redeemed profile reflects complimentary Power access, deactivate the second code from the live admin UI, and verify the second QA user is blocked on the live Settings page with the inactive-code message
- **Expected Result:** The live public frontend exposes the new admin access-code UI, the admin can issue and manage codes entirely through the public host, one QA redemption succeeds and reflects `free_premium_access=true`, `subscription_status=active`, and `subscription_tier=power` on the redeemed profile, and a deactivated code is blocked on the live host with the exact inactive-code message
- **Actual Result:** Passed. The clean production deploy landed on the real `coparrent` Vercel project and both `https://www.coparrent.com/login` and `https://coparrent.com/login` returned fresh production HTML immediately after the deploy. The public host exposed the `Access Codes` tab, the admin minted the QA batch with one-time raw reveal, both previews appeared in inventory, the redeem QA user successfully activated complimentary Power access from the live Settings page, and the inactive QA user was blocked with the exact message `That access code is no longer active.`
- **Pass / Fail:** Passed

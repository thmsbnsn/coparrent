...
export const GameCard = ({
  ...
  <article
    className={cn(
      "relative overflow-hidden rounded-[2rem] border border-border/70 bg-card/90 p-5 shadow-[0_24px_50px_-36px_rgba(15,23,42,0.75)]",
      className,
    )}
  >
    ...
    <div className="relative flex h-full flex-col gap-5 text-white">
      ...
      <div className="mt-auto flex items-end justify-between gap-4">
        <Link
          to={to}
          className="inline-flex h-12 items-center rounded-full bg-white px-5 text-sm font-semibold text-slate-950 shadow-sm transition hover:bg-slate-100"
        >
          {actionLabel}
        ...
        {artSrc ? (
          <img
            alt={artAlt}
            src={artSrc}
            className={cn("pointer-events-none select-none drop-shadow-[0_18px_20px_rgba(15,23,42,0.18)]", artClassName)}
          />
        ) : null}
      </div>
    </div>
